...
export default async function handler(req, res) {
  ...
  try {
    ...
    // narração é tratada por história (fica para a peça de áudio); aqui só marcamos se veio sozinha

    await sbPatch(SUPABASE_URL, SERVICE_ROLE,
      `/rest/v1/perfis?id=eq.${id}`, update);

    return res.status(200).json({ ok: true, email, aplicado: update });
  } catch (err) {
